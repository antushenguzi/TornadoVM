...
def runBenchmarksFullCoverage(args):
    if args.benchmark and args.benchmark not in allSizes:
        print(f"Error: '{args.benchmark}' does not match a valid key in allSizes.")
        return
    ...
    for key in allSizes.keys():
        ...
        for size in allSizes[key][0]:
            if args.threads and size != args.threads:
                continue  # Run command

            numIterations = eval(allSizes[key][1][0])  # interations
            command = (
                __TORNADO_COMMAND__
                + tornado_options
                + ' --jvm="'
...
def parseArguments():
    ...
    dest="threads",
    default=None,
    help="Specify the number of threads for execution"
    )
